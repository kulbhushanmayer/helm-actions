...
# Prefer provided token, else fallback to GITHUB_TOKEN env available in runner
...
export IMAGE_TAGS="${GITHUB_RUN_ID}-${SHORT_SHA}"
export FINAL_VERSION="${VERSION_PREFIX}-${IMAGE_TAGS}"

echo "Computed IMAGE_TAGS=${IMAGE_TAGS}"
echo "Computed FINAL_VERSION=${FINAL_VERSION}"

# ---- update values & Chart.yaml ----
...
if [ "${PUSH_CHART}" = "true" ] || [ "${PUSH_CHART}" = "True" ]; then
  ...
  # push (basename required by helm push when packaging from other dir)
  helm push "$(basename "${CHART_TGZ}")" "oci://${OCI_REGISTRY}/${OCI_REPOSITORY}"
  # logout
